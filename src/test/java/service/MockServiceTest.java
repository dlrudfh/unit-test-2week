...
public class MockServiceTest {
    ...
    public void 챔피언이름을가져오면_무조건_카이사를_리턴한다() {
        ...
        //(champion.getName()).thenReturn("카이사");
    ...
    @Test
    public void 어떠한포지션을set해도무조건미드리턴테스트() {
        Champion champion = mock(Champion.class);
    ...
    @Test
    public void 객체에서이름을가져오는로직이2번이상실행되면Pass(){
        Champion champion = mock(Champion.class);
    ...
    @Test
    public void 객체에서이름을가져오는로직이3번이하실행되면Pass(){
        Champion champion = mock(Champion.class);
    ...
    @Test
    public void 객체에서이름을가져오는로직이실행되지않았으면Pass(){
        Champion champion = mock(Champion.class);
        champion.setName("야스오");
        champion.setPosition("탑");
        champion.setHasSkinCount(3);
    ...
    @Test
    public void 객체에서이름을가져오는로직이200ms이내에1번실행되었는지검증(){
        Champion champion = mock(Champion.class);
        champion.setName("리븐");
        champion.setPosition("탑");
        champion.setHasSkinCount(3);
        // times는 정확히 n번, atLeast는 최소 n번
    ...
    @Test
    public void 가짜챔피언_메소드1번_스킨갯수검증() {
        when(mockService.findByName(anyString())).thenReturn(new Champion("요네", "미드", 2));
        assertThat(mockService.findByName("니코").getHasSkinCount(), is(3));
        verify(mockRepository, times(1)).findByName(anyString());
    }
    ...
    public void 가짜챔피언객체리턴() {
        ...
        champions.add(new Champion("이즈리얼", "원딜", 3));
        champions.add(new Champion("징크스", "원딜", 2));
        when(mockService.findAllChampions()).thenReturn(champions);
